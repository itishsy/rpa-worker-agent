...
项目已有 `IVmOperationLock`，快照切换和画像升级都按 `VmxPath` 获取同一把锁。健康检测已经改为纯观察，不参与电源恢复。磁盘维护实现必须复用该锁，并从空闲确认开始一直持有到维护结束；如果未来维护流程包含开关机，还需持有到 VM 恢复启动且完成基本健康验证。
...
10. 维护功能关闭时，不改变现有调度、切换和纯观察健康检测行为。
